...
- **Lighthouse on the live deployment, three runs of each profile: desktop 99 to 100 performance with a 0.8 s largest paint, mobile on a throttled connection 91 to 96 with a 3.4 s largest paint. Both profiles 100 accessibility, 100 best practices and 100 SEO, with layout shift between 0 and 0.024 against a 0.1 threshold.** The range is published rather than the best run, and `npm run audit` reproduces it.
