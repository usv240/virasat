    ...
    <>
      <Section eyebrow="For developers" title="Use Virasat as a service" h1>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[["Sandbox key", "vs_test_demo", "Free. Sample data and your own files. 30 requests a minute."], ["Live keys", "vs_live_...", "For institutions. Stored only as a SHA-256 hash on the server. Scopes and limits per key."], ["Errors", "application/problem+json", "Every error follows RFC 9457 with a plain-language detail field. Rate limit headers on every response."], ["Limits, honestly", "X-Virasat-RateLimit-Store", "Every response says which counter it used. shared means Upstash Redis, one counter across every instance, one round trip. instance means that store was not configured or did not answer within 1.5 seconds, and the limit fell back to this instance's memory, which is weaker than the number suggests. We fail open rather than fail a family's claim because a cache blinked."]].map(([h, code, d]) => (
...
# 1. Route a claim (rules plus the AI Review)
      ...
      <Section eyebrow="Bring your own" title="Your data, your key, your rules" tone="surface">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card><h3 className="text-lg text-brand">Bring Your Own Rules</h3><p className="mt-2 text-sm">Pass a <code>rules</code> array to <code>/claims/route</code>, or validate one first with <code>POST /rules</code>. Every result records the rule id and version used, so your compliance team can audit it.</p>
            <pre tabIndex={0} className="mt-2 overflow-x-auto rounded bg-surface p-2 text-xs"><code>{`{"institution":"My Bank","assetType":"bank","version":1,
