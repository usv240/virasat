...
export async function POST(req: Request) {
  ...
  const { input, extraction, userNotes, debate, rules } = parsed.data;

  const decision = decide(input, rules ?? []);
  if (!debate) return withHeaders(Response.json({ decision, debate: null, mode: "rules" }), g.info, g.rl);

  const byo = keyFromRequest(req);
  if (!hasKey(byo)) {
    return withHeaders(Response.json({ decision, debate: { ...SAMPLE_DEBATE, verdict: { ...SAMPLE_DEBATE.verdict, rule_ids_checked: [decision.ruleId] } }, mode: "sample" }), g.info, g.rl);
  }
  try {
    const result = await runDebate({ extraction: extraction ?? null, input, decision, userNotes }, byo);
