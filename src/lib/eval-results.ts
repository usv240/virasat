...
export const EVAL_RESULTS = {
  ...
  rows: [
    ...
    { test: "Dividend list parser: real published lists", cases: "3 company lists, 1263 pages, downloaded at check time and never stored", result: "46,282 rows parsed, search finds 100 percent of a 1128 row sample", note: "Real unpaid-dividend lists that companies must publish under the Companies Act. Counts only; no name is kept. On the 974 page list the company prints its own total on the cover, 49,52,460.80 rupees; the parser sums 49,52,209, so it misses about two rows in 23,709. Reproduce with npm run check:dividends." },
